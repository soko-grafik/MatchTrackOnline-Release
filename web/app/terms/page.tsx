...
import { ChevronLeft, Edit3, Printer, FileText } from 'lucide-react';
...
export default function TermsPage() {
  ...
  useEffect(() => {
    getPublicLegalPages()
      .then((data) => {
        if (data && data.terms_content) {
          setTermsContent(data.terms_content);
        }
      })
      .catch((err) => {
        console.error('Fehler beim Laden der Nutzungsbedingungen:', err);
      })
      .finally(() => {
        setLoading(false);
      });
    ...
    <div className="min-h-screen bg-zinc-950 text-white selection:bg-primary/30 selection:text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10">
        <div className="bg-zinc-900/60 border border-zinc-800 rounded-3xl p-6 sm:p-10 shadow-2xl backdrop-blur-sm">
          <div className="flex items-center gap-3 mb-6 pb-4 border-b border-zinc-800/80 text-purple-400">
            <div className="p-3 rounded-2xl bg-purple-500/10 border border-purple-500/20">
              <FileText className="w-6 h-6 text-purple-400" />
            </div>
            <div>
              <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Terms of Service</span>
              <h1 className="text-xl sm:text-2xl font-black text-white">Allgemeine Nutzungsbedingungen</h1>
            </div>
          </div>
          ...
          {loading ? (
            <div className="py-16 text-center space-y-3">
              ...
              <p className="text-xs text-zinc-400">Lade Nutzungsbedingungen...</p>
            </div>
          ) : (
            <MarkdownRenderer content={termsContent} className="space-y-4" />
          )}
